  ...
  u_int32_t packet_id)
{
  ...
  {
    bool src_to_dst_direction = true;
#ifdef DEBUG_PAYLOAD
    printf(
      "[hashval: %u][proto: %u][vlan: %u][%s:%u <-> %s:%u][direction: %s][payload_len: %u]\n",
      flow->hashval,
      flow->protocol,
      flow->vlan_id,
      flow->src_name,
  ...
  {
    for (int j = min_pattern_len; j <= max_pattern_len; j++)
    {
      if (i + j < payload_len)
      {
...
int payload_stats_sort_asc(void *_a, void *_b)
{
...
void print_payload_stat(struct payload_stats *p, FILE *out)
{
  u_int i;
...
uint16_t ndpi_get_proto_id(struct ndpi_detection_module_struct *ndpi_mod, const char *name)
{
  uint16_t proto_id;
  char *e;
  ...
  struct ndpi_global_context *g_ctx)
{
  ...
  workflow = (ndpi_workflow*)ndpi_calloc(1, sizeof(struct ndpi_workflow));
  if (workflow == NULL)
  {
    LOG(NDPI_LOG_ERROR, "global structure initialization failed\n");
    ndpi_exit_detection_module(module);
  ...
  u_int8_t src_to_dst_direction)
{
  ...
  if ((flow->entropy->src2dst_pkt_count + flow->entropy->dst2src_pkt_count) <= max_num_packets_per_flow)
  {
    ...
    {
      const unsigned char *data = (const unsigned char*)x;
 ...
 */
void
ndpi_flow_update_byte_dist_mean_var(
...
struct ndpi_flow_info* get_ndpi_flow_info(
  struct ndpi_workflow * workflow,
  ...
  *proto = iph->protocol;
  ...
  if (ret == NULL)
  {
	...
	inet_ntop(AF_INET, &newflow->dst_ip, newflow->dst_name, sizeof(newflow->dst_name));
      ...
      if ((newflow->ndpi_flow = (ndpi_flow_struct*)ndpi_flow_malloc(SIZEOF_FLOW_STRUCT)) == NULL)
      {
	...
	memset(newflow->ndpi_flow, 0, SIZEOF_FLOW_STRUCT);
      ...
      if (ndpi_tsearch(newflow, &workflow->ndpi_flows_root[idx], ndpi_workflow_node_cmp) == NULL)
      {
        // Add
        ndpi_flow_info_free_data(newflow);
        ndpi_free(newflow);
      ...
      {
        newflow->entropy = (struct ndpi_entropy*)ndpi_calloc(1, sizeof(struct ndpi_entropy));
        newflow->last_entropy = (struct ndpi_entropy*)ndpi_calloc(1, sizeof(struct ndpi_entropy));
...
struct ndpi_flow_info* get_ndpi_flow_info6(
  struct ndpi_workflow * workflow,
  u_int16_t vlan_id,
  ndpi_packet_tunnel tunnel_type,
  const struct ndpi_ipv6hdr *iph6,
...
void add_to_address_port_list(ndpi_address_port_list *list, ndpi_address_port *ap)
{
  void *new_buf;
  ...
  for (unsigned int i = 0; i < list->num_aps; i++)
  {
  ...
  {
    int new_num = 1 + list->num_aps_allocated * 2;
    new_buf = ndpi_realloc(
      list->aps, list->num_aps_allocated * sizeof(ndpi_address_port),
      new_num * sizeof(ndpi_address_port));
    if (!new_buf)
      return;
    list->aps = (ndpi_address_port*)new_buf;
...
void process_ndpi_monitoring_info(struct ndpi_flow_info *flow)
{
  if (!flow->ndpi_flow || !flow->ndpi_flow->monit)
    return;

  if (flow->monitoring_state == 0 && flow->ndpi_flow->monitoring)
  {
    // We just moved to monitoring state
    flow->monitoring_state = 1;
    flow->num_packets_before_monitoring = flow->ndpi_flow->packet_direction_complete_counter[0] +
      flow->ndpi_flow->packet_direction_complete_counter[1];
  }
...
void serialize_monitoring_metadata(struct ndpi_flow_info *flow)
{
  unsigned int i;
  char buf[64];
  ...
  switch(flow->detected_protocol.proto.master_protocol ? flow->detected_protocol.proto.master_protocol :
    ...
    case NDPI_PROTOCOL_SRTP:
      ...
      if (flow->stun.peer_address.num_aps > 0)
      {
        ...
        for (i = 0; i < flow->stun.peer_address.num_aps; i++)
        {
          ...
          {
            ndpi_serialize_string_string(
              &flow->ndpi_flow_serializer, "peer_address",
      ...
      if (flow->stun.response_origin.num_aps > 0)
      {
        ...
        for (i = 0; i < flow->stun.response_origin.num_aps; i++)
        {
          ...
          {
            ndpi_serialize_string_string(
              &flow->ndpi_flow_serializer, "response_origin",
...
void process_ndpi_collected_info(struct ndpi_workflow * workflow, struct ndpi_flow_info *flow)
{
    ...
    !is_ndpi_proto(flow, NDPI_PROTOCOL_TLS))
  {
    if (flow->ndpi_flow->protos.bittorrent.hash[0] != '\0')
    {
      ...
      flow->bittorent_hash = (char*)ndpi_malloc(avail);

      if (flow->bittorent_hash)
      {
        u_int j = 0;
        for (int i = 0; i < sizeof(flow->ndpi_flow->protos.bittorrent.hash); i++)
        {
...
*/
static struct ndpi_proto packet_processing(
  struct ndpi_workflow * workflow,
  ...
  struct ndpi_flow_info **flow_ext)
{
  ...
  if (!flow->detection_completed)
  {
    ...
    u_int enough_packets =
      ((proto == IPPROTO_UDP && (max_num_udp_dissected_pkts > 0 &&
        flow->src2dst_packets + flow->dst2src_packets > max_num_udp_dissected_pkts)) ||
       (proto == IPPROTO_TCP && (max_num_tcp_dissected_pkts > 0 &&
        flow->src2dst_packets + flow->dst2src_packets > max_num_tcp_dissected_pkts))) ?
      1 : 0;
...
static bool ndpi_is_valid_vxlan(
  const struct pcap_pkthdr *header, const u_char *packet, u_int16_t ip_offset, u_int16_t ip_len)
{
...
static uint32_t ndpi_is_valid_gre_tunnel(
  const struct pcap_pkthdr *header,
  ...
  int datalink_type)
{
 ...
 iph_check:
  ...
  return packet_processing(
    workflow, time_ms, vlan_id, tunnel_type, iph, iph6,
...
int dpdk_port_init(int port, struct rte_mempool *mbuf_pool)
{
  ...
  for (u_int16_t q = 0; q < rx_rings; q++)
  {
  ...
  for (u_int16_t q = 0; q < tx_rings; q++)
  {
