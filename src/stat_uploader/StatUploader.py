...
class Config :
  ...
  def init_json(self, config_json) :
    stat_json = config_json.get('stat', {})
    self.pid_file = stat_json.get('pid_file', None)
    self.clickhouse_conn = stat_json.get('clickhouse_conn', '')
    self.check_roots = stat_json.get('check_roots', [])
    if 'dump_stat_root' in config_json:
        self.check_roots.append(config_json['dump_stat_root'])
    self.error_root = stat_json.get('error_root', None)
    self.batch = stat_json.get('batch', 1000)
...
"""
DetailedTrafficStatsUploader: uploaded for DetailedStat logs.
"""
class DetailedTrafficStatsUploader(object) :
  clickhouse_conn : str
  ...
  def __init__(self, config, logger = None) :
    ...
    self.command_line_templ = jinja2.Template(
      "cat {{ process_files|join(' ') }} | clickhouse-client {{clickhouse_conn}} " +
      '--query="INSERT INTO detailed_traffic_stats FORMAT CSV"')
    self.logger = logger
...
"""
DetailedEventStatsUploader: uploaded for DetailedStat logs.
"""
class DetailedEventStatsUploader(object) :
  clickhouse_conn : str
  command_line_templ : jinja2.Template
  logger = None
  ...
  def __init__(self, config, logger = None) :
    ...
    self.command_line_templ = jinja2.Template(
      "cat {{ process_files|join(' ') }} | clickhouse-client {{clickhouse_conn}} " +
      '--query="INSERT INTO detailed_event_stats FORMAT CSV"')
    self.logger = logger
...
def main() :
  parser = argparse.ArgumentParser(description = 'StatUploader.')
  parser.add_argument("-c", "--config", type = str, default = "./stat_uploader.conf")
  ...
  processors = {}
  processors['DetailedTrafficStats'] = DetailedTrafficStatsUploader(config, logger = logger)
  processors['DetailedEventStats'] = DetailedEventStatsUploader(config, logger = logger)
