#ifndef GEARS_COMMON_DECIMAL_HPP
#define GEARS_COMMON_DECIMAL_HPP
...
namespace Gears
{
   ...
   */
  DECLARE_EXCEPTION(DecimalException, Gears::DescriptiveException);
  ...
  void
  narrow_decimal(SimpleDecimal& s, const Decimal& d) /*throw (Gears::Exception)*/
  {
  ...
  SimpleDecimal
  convert_float(Float value) /*throw (Gears::Exception)*/
  {
    static const char* FUN = "convert_float()";
    ...
    if (state != FP_NORMAL)
    {
      ...
      ostr << FUN << ": provided value is not normal";
      throw typename SimpleDecimal::Exception(ostr.str());
    }
    const int64_t POW =
    ...
    {
      Gears::ErrorStream ostr;
      ostr << FUN << ": provided value is too big";
      throw typename SimpleDecimal::Overflow(ostr.str());
    }
