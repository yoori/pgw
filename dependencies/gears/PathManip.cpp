...
namespace PathManip
{
  ...
  /*
  std::string
  uri_to_fpath(const Gears::SubString& uri)
    noexcept
  {
    try
    {
      HTTP::URLAddress address(uri);
      if (address.scheme() == Gears::AsciiStringManip::Caseless("file"))
      {
  ...
  */
  ...
  {
    typedef std::list<Gears::SubString> PartList;
