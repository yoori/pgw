#ifndef GEARS_DIRSELECTOR_HPP_
#define GEARS_DIRSELECTOR_HPP_
  ...
  namespace DirSelect
  {
      ...
      /*throw (Exception)*/
    {
      Gears::throw_errno_exception<FailedToOpenDirectory>(
        "default_failed_to_open_directory(): ", "'", full_path, "'");
    }
    ...
    namespace DirectoryFilterHelper
    {
        ...
        ErrorHandler failed_to_stat_file) /*throw (Gears::Exception)*/
      {
        ...
        std::vector<char> full_path(path_len + NAME_MAX + 2);
        if (path_len)
        {
        ...
        {
          if (Gears::StringManip::strlcpy(name, entry->d_name, NAME_MAX)
              >= NAME_MAX)
          {
            //TODO
          }
